...
public class CardBusiness {
    ...
    public CardInfo getCardInfo() throws ReadCardException {
        ...
        if (TextUtils.isEmpty(response) || !response.endsWith("9000") || response.length() < 120) {
...
//        }
    ...
    public String getData0015() {
        String response;
        sendApdu(makeCmd("00A4040009A00000000386980702"), 24);
        response = sendApdu(makeCmd("00B095000032"), 16);
        if (response != null && response.endsWith("9000")) {
            int length = response.length();
            response = response.substring(0, length - 4);
//            String beforeBase = response.substring(0, 20);
//            String base = response.substring(20, 24);
//            String afterBase = response.substring(24, length - 4);
//            String data0015new = beforeBase + "0064" + afterBase;
            return response;
        }
        return "";
    }

    public String getData0015New() {
        String response;
        sendApdu(makeCmd("00A4040009A00000000386980702"), 24);
        response = sendApdu(makeCmd("00B095000032"), 16);
        if (response != null && response.endsWith("9000")) {
    ...
    public String sendPdus(String[] pdus, boolean isOriginal) {
        String response = "";
        if (isOriginal) {//命令已经组好
            for (int i = 0; i < pdus.length; i++) {
                response = sendApdu(ByteUtil.hexStr(pdus[i]), pdus[i].length() / 2);
            }
        } else {
            for (int i = 0; i < pdus.length; i++) {
                response = sendApdu(makeCmd(pdus[i]), pdus[i].length() / 2 + 10);
            }
        }
        return response;
    }
     ...
     * 圈存初始化
     */
    public CardInfo getTopInitInfo(CardInfo mCardInfo, String reloadBal, String deviceId, int cardType) throws Exception {
        Log.i(TAG, "card type ------ ========= " + cardType);
        if (cardType == ConstantMsg.CARD_TYPE_TRANSPORT) {
            sendApdu(makeCmd("00A4040008A000000632010105"), 23);
        } else {
